...
namespace fs_ops::executor {
  ...
  auto execution_report::failure_count() const noexcept -> std::int64_t { return std::ssize(failures_); }

  auto execution_report::skipped_count() const noexcept -> std::int64_t {
    return processed_count_ - success_count_ - failure_count();
  }

  auto execution_report::failures() const noexcept -> std::span<const fs_ops::failed_operation> { return failures_; }
} // namespace fs_ops::executor
