...
namespace {
  ...
  // TODO: Separate collision resolver
  ...
  auto make_candidate_path(const candidate& c, int idx) -> fs::path {
    ...
    const auto extension{c.extension};
    const auto stem{c.stem};
    return {parent / fmt::format("{} ({}){}", stem, idx, extension)};
  }

  auto make_candidate_paths(const candidate& c) -> std::vector<fs::path> {
    constexpr auto max_candidate_index{100};
  ...
  auto process_operation(const successful_operation& op) -> operation_result {
    ...
    return [result{perform_move(op)}, &op] {
      return result.has_value() ? operation_result::create_success()
